    ...
    def compute_dfg(self, df: pd.DataFrame, include_performance: bool = True) -> tuple[List[Dict[str, object]], List[str], List[str]]:
        ...
        for (source, target) in pairs:
            ...
            def _format_duration(seconds_value: float) -> str:
                seconds_value = float(seconds_value or 0)
                days = seconds_value / 86400.0
                if days >= 1:
                    formatted = f"{days:.1f}".replace('.', ',')
                    return f"{formatted} hari"
                hours = seconds_value / 3600.0
                if hours >= 1:
                    formatted = f"{hours:.1f}".replace('.', ',')
                    return f"{formatted} jam"
                minutes = seconds_value / 60.0
                formatted = f"{minutes:.1f}".replace('.', ',')
                return f"{formatted} menit"

            median_string = _format_duration(median)
            mean_string = _format_duration(mean)
