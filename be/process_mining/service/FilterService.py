...
@dataclass
class VariantCoverageFilter(FilterService):
...
@dataclass
class EventCoverageFilter(FilterService):
...
@dataclass
class CaseDurationFilter(FilterService):
...
@dataclass
class CaseSizeFilter(FilterService):
    min_size: int
    max_size: int
    def apply(self, event_log: EventLog) -> EventLog:
        df = event_log.df
